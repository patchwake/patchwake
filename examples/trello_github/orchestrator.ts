...
import type { AgentRuntime } from "../../orchestra/ports.js";
import { FileStateStore, TickAlreadyRunning } from "../../orchestra/state.js";
import { errorMessage } from "../../orchestra/json.js";
...
export function buildEngine({
  ...
  const engineName = (env.ORCHESTRA_AGENT_ENGINE ?? "claude").toLowerCase();
  const common = {
    model: env.ORCHESTRA_MODEL ?? "",
    passthroughEnv: list(env.ORCHESTRA_AGENT_ENV),
  };
  ...
  if (engineName === "claude")
    runtime = new ClaudeCodeRuntime(store, {
      ...
      permissionMode: env.ORCHESTRA_PERMISSION_MODE ?? "default",
    });
  else if (engineName === "codex") {
    const sandbox = env.ORCHESTRA_CODEX_SANDBOX ?? "workspace-write";
    if (
      sandbox !== "read-only" &&
    ...
    const config: CodexRuntimeConfig = {
      ...
      sandbox,
      networkAccess: (env.ORCHESTRA_CODEX_NETWORK_ACCESS ?? "1") === "1",
      dangerouslyBypassApprovalsAndSandbox:
        env.ORCHESTRA_CODEX_DANGEROUS_BYPASS === "1",
    };
    runtime = new CodexCliRuntime(store, config);
  } else
    throw new Error(
      `ORCHESTRA_AGENT_ENGINE must be 'claude' or 'codex', not '${engineName}'`,
    );
  return new Engine({
    board,
    ...
    config: {
      maxConcurrentTurns: integer(env, "ORCHESTRA_MAX_TURNS", 3),
      stallAfterSeconds: integer(env, "ORCHESTRA_STALL_AFTER", 900),
      maxRestarts: integer(env, "ORCHESTRA_MAX_RESTARTS", 5),
      requireDirectedActivity: (env.ORCHESTRA_REQUIRE_DIRECTED ?? "1") === "1",
...
export async function main(args = process.argv.slice(2)): Promise<number> {
  ...
  if (values.help) {
    console.log(
      "Usage: orchestra-trello-github [--dry-run] [--status]\nRun one Trello + GitHub orchestration tick.",
    );
    return 0;
  }
